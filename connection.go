...
func NewConnection(ts oauth2.TokenSource, opts ...Option) (*Connection, error) {
	...
	// Init the cacheController if not disabled
	if !conn.cacheDisabled {
		conn.cacheController = *conn.initCacheController()
	}

	return conn, nil
}

func (conn *Connection) initCacheController() *cacheController {
	cacheController := &cacheController{
	...
	cacheController.homesAndSystemsCache = ResettableCached(func() (HomesAndSystems, error) {
...
// Internal function that reads all homes
func (c *Connection) getHomes(res *Homes) error {
	url := API_URL_BASE + "/homes"
	req, _ := http.NewRequest("GET", url, nil)

	// var res Homes
	if err := doJSON(c.client, req, &res); err != nil {
		return fmt.Errorf("error getting homes: %w", err)
	}
	return nil
}

// Internal function that returns the system report (state, properties and configuration) for a specific systemId
func (c *Connection) getSystem(systemid string) (SystemStatus, error) {
	...
	if err := doJSON(c.client, req, &state); err != nil {
		return state, fmt.Errorf("error getting state for system id %s: %w", systemid, err)
	}
	return state, nil
...
func (c *Connection) refreshCurrentQuickMode(state *SystemStatus) {
	...
	if newQuickMode != c.cacheController.currentQuickmode {
		if newQuickMode == "" && time.Now().After(c.cacheController.quickmodeStarted.Add(c.cacheController.cache)) {
			if c.cacheController.currentQuickmode == QUICKMODE_NOTHING && time.Now().Before(c.cacheController.quickmodeStarted.Add(10*time.Minute)) {
				c.debug("Idle mode active for less then 10 minutes. Keeping the idle mode")
			} else {
				c.debug(fmt.Sprintf("Old quickmode: \"%s\"   New quickmode: \"%s\"", c.cacheController.currentQuickmode, newQuickMode))
				c.cacheController.currentQuickmode = newQuickMode
				c.cacheController.quickmodeStopped = time.Now()
			}
		}
		if newQuickMode != "" && time.Now().After(c.cacheController.quickmodeStopped.Add(c.cacheController.cache)) {
			c.debug(fmt.Sprintf("Old quickmode: \"%s\"   New quickmode: \"%s\"", c.cacheController.currentQuickmode, newQuickMode))
			c.cacheController.currentQuickmode = newQuickMode
			c.cacheController.quickmodeStarted = time.Now()
		}
	}
}
...
func (c *Connection) GetHomes() (Homes, error) {
	if c.cacheDisabled {
		var homes Homes
		err := c.getHomes(&homes)
		return homes, err
	} else {
		homesAndSystems, err := c.cacheController.homesAndSystemsCache.Get()
		if err != nil {
...
func (c *Connection) StartStrategybased(systemId string, strategy int, heatingPar *HeatingParStruct, hotwaterPar *HotwaterParStruct) (string, error) {
	if c.cacheDisabled {
		...
		if c.cacheController.currentQuickmode != "" {
			c.debug(fmt.Sprint("System is already in quick mode:", c.cacheController.currentQuickmode))
		...
		case 1:
			...
			if err == nil {
				c.cacheController.currentQuickmode = QUICKMODE_HOTWATER
				c.cacheController.quickmodeStarted = time.Now()
		...
		case 2:
			...
			if err == nil {
				c.cacheController.currentQuickmode = QUICKMODE_HEATING
				c.cacheController.quickmodeStarted = time.Now()
				c.debug("Starting zone quick veto")
			}
		default:
			if c.cacheController.currentQuickmode == QUICKMODE_HOTWATER {
			...
			if c.cacheController.currentQuickmode == QUICKMODE_HEATING {
			...
			c.cacheController.currentQuickmode = QUICKMODE_NOTHING
			c.cacheController.quickmodeStarted = time.Now()
			c.debug("Enable called but no quick mode possible. Starting idle mode")
		}

		c.cacheController.homesAndSystemsCache.Reset()
		return c.cacheController.currentQuickmode, err
	}
}
...
func (c *Connection) StopStrategybased(systemId string, heatingPar *HeatingParStruct, hotwaterPar *HotwaterParStruct) (string, error) {
	if c.cacheDisabled {
		...
		switch c.cacheController.currentQuickmode {
		case QUICKMODE_HOTWATER:
			err = c.StopHotWaterBoost(systemId, hotwaterPar.Index)
			if err == nil {
				c.debug(fmt.Sprint("Stopping quick mode", c.cacheController.currentQuickmode))
			}
		...
		c.cacheController.currentQuickmode = ""
		c.cacheController.quickmodeStopped = time.Now()
...
func (c *Connection) WhichQuickMode(dhwData *DhwData, zoneData *ZoneData, strategy int, heatingPar *HeatingParStruct, hotwater *HotwaterParStruct) int {
	if c.cacheDisabled {
		c.debug("Function WhichQuickMode() not supported if option disableCache active")
		return -1
	} else {
